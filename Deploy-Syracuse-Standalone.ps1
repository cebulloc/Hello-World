...
<#
.SYNOPSIS
    Syracuse - Standalone / Desktop Deployment
.DESCRIPTION
    Drop this script into the Installers folder on the user's Desktop
    alongside all staged installer files (flat - no subfolder needed):

      Desktop\
        Installers\
          Deploy-Syracuse-Standalone.ps1   <- this file
...
$instDir   = "$env:TEMP\Syracuse_SA_Installers"
$logFile   = "$env:TEMP\Syracuse_Deploy_$($env:COMPUTERNAME)_$(Get-Date -Format yyyyMMdd).log"
$lockPath  = "$env:TEMP\Syracuse_Deploy_$($env:COMPUTERNAME).lock"
$donePath  = "$env:TEMP\Syracuse_Deploy_$($env:COMPUTERNAME)_COMPLETE.flag"
...
Syracuse Deployment Log (Standalone)
...
Write-Log '================================================================'
Write-Log "Syracuse Deployment (Standalone) - $env:COMPUTERNAME"
...
Invoke-Section '[1/12] Miniforge3 (Python 3.13 + packages)' {
    ...
    if (Test-Path "$dest\Scripts\conda.exe") {
        ...
        Write-Log "    Running package installs anyway in case they are missing..."
    } else {
    ...
    & "$dest\Scripts\conda.exe" install --yes numpy scipy pandas matplotlib plotly dash seaborn simpy `
        2>&1 | Add-Content $logFile -Encoding UTF8
    Write-Log '    conda packages done.'

    Write-Log '    Installing OpenMDAO via pip...'
    & "$dest\python.exe" -m pip install openmdao 2>&1 | Add-Content $logFile -Encoding UTF8
    Write-Log '    pip packages done.'
...
Invoke-Section '[9/12] Poetry (Python dependency manager)' {
    ...
    $installer = Get-ChildItem "$instDir\install-poetry.py" -ErrorAction SilentlyContinue |
                 Select-Object -First 1
    if ($installer) {
        Write-Log '    Using staged install-poetry.py.'
        $pyScript = $installer.FullName
    } else {
        if ($StagedOnly) { Write-Log '    [SKIP] Not staged and StagedOnly is set.'; return }
        Write-Log '    Not staged - downloading installer script...'
        $pyScript = "$env:TEMP\install-poetry.py"
    ...
    $proc = Start-Process -FilePath $python -ArgumentList $pyScript -Wait -PassThru
    Write-Result $proc.ExitCode
    ...
    Write-Log '    Open a new terminal to use the poetry command.'
}
...
Write-Host '  1. Open a NEW terminal to use conda / Python 3.13 and poetry'
Write-Host '  2. Verify packages: python -c "import openmdao, numpy, scipy, pandas, dash; print(ok)"'
Write-Host '  3. VS Code Remote-SSH: F1 > "Remote-SSH: Add New Host" - SSH wrapper pre-configured'
Write-Host '  4. TortoiseGit: shell icons may need Explorer restart'
Write-Host '  5. MikTeX: run MikTeX Console after first launch to update packages'
