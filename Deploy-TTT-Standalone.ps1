...
<#
.SYNOPSIS
    TTT Pathways - Standalone / Desktop Deployment
.DESCRIPTION
    ...
    alongside all the staged installer files (flat - no subfolder needed):
      ...
      Desktop\
        Installers\
          Deploy-TTT-Standalone.ps1   <- this file
          Miniforge3-Windows-x86_64.exe
          VSCodeSetup-x64.exe
          ssh-wrapper.bat
          basic-miktex-x.x-x64.exe
          MobaXterm_installer_x.y.msi
          OpenVSP-3.50.4-win64-Python3.13.zip
          Git-*-64-bit.exe
...
$instDir   = "$env:TEMP\TTT_SA_Installers"
...
TTT Pathways Deployment Log (Standalone)
...
Write-Log '================================================================'
Write-Log "TTT Pathways Deployment (Standalone) - $env:COMPUTERNAME"
...
Invoke-Section "[9/12] OpenVSP $VSP_VER" {
    ...
    $installer = Get-ChildItem "$instDir\OpenVSP-$VSP_VER*win64*.zip" -ErrorAction SilentlyContinue |
                 Select-Object -First 1
    if ($installer) {
